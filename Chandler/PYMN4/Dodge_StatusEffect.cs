...
namespace PYMN4
{
  public class Dodge_StatusEffect : IStatusEffect, ITriggerEffect<IStatusEffector>
  {
    ...
    {
      CombatManager.Instance.AddObserver(new Action<object, object>(this.OnStatusTick), Dodge.Call.ToString(), (object) caller);
      CombatManager.Instance.AddObserver(new Action<object, object>(this.OnStatusTick), ((TriggerCalls) 21).ToString(), (object) caller);
    }

    public void OnTriggerDettached(IStatusEffector caller)
    {
      CombatManager.Instance.RemoveObserver(new Action<object, object>(this.OnStatusTick), Dodge.Call.ToString(), (object) caller);
      CombatManager.Instance.RemoveObserver(new Action<object, object>(this.OnStatusTick), ((TriggerCalls) 21).ToString(), (object) caller);
    }
