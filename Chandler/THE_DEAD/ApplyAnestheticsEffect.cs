...
namespace THE_DEAD
{
  public class ApplyAnestheticsEffect : EffectSO
  {
      ...
      out int exitAmount)
    {
      ...
      stats.statusEffectDataBase.TryGetValue((StatusEffectType) 204308, out statusEffectInfoSo);
      stats.statusEffectDataBase.TryGetValue((StatusEffectType) 11, out StatusEffectInfoSO _);
      for (int index1 = 0; index1 < targets.Length; ++index1)
      {
        if (targets[index1].HasUnit)
        {
          ...
          IStatusEffect istatusEffect = (IStatusEffect) new Anesthetics_StatusEffect(amount);
